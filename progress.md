...
- **In review:** [PR 10](https://github.com/taia-0/Open-Era/pull/10), `feature/player-trade-verbs` (M16) at `ade42be`, verified and with its playtest record written ([trade-voyage-001](docs/playtests/trade-voyage-001.md), `PROMOTE` for the solvent-voyage hypothesis). 122 tests pass, typecheck is clean, the golden hashes are unchanged, and the prior merges onto `main` (`deeb0d1`, [PR 8](https://github.com/taia-0/Open-Era/pull/8)) are untouched by it. The playtest's four projection and wording defects are closed on this branch before review; its design findings are recorded in [open items](#open-items) as the economy milestone's brief, and none of them is a simulation transition.
