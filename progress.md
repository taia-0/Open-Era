...
- **Headline risk:** no channel exists for a commander to learn a rival's strength other than a settlement-scoped forecast. The feed's missing history, the missing pre-commitment forecast, and a party's blindness to its own provisioning are all closed as of Milestones A and B; estimation through investigation is not. A second risk is now visible behind it: the design record never describes a player-facing trade loop, and the implementation cannot support one, so the maritime economy the world's markets imply has no player verb.
...
| `POST /api/briefing/acknowledge` accepts an `itemId` that names no current item and answers `{ ok: true }` | Defect | Unassigned | Open, needs a decision. Acknowledging a nonexistent or already-superseded id emits an event and reports success, so a client cannot tell a real acknowledgement from a typo. It is now documented as a deliberate no-op so a stale click after the tick advances is not an error; tightening it would reject those clicks instead |
| `trade-local` takes no parameters, returns only `{ok, command}`, and silently liquidates whatever it chooses | Defect | Unassigned | Open; found by the own-party session, which called it three times and had 5 ship materials, then 13.6 provisions, then 9.568 provisions sold without ever being told which good or at what price |
| `buy-provisions` has no quantity or cost preview and can spend an entire treasury in one command, and its `insufficient-money` prose conflates the 2-money minimum balance with the price | Defect | Unassigned | Open; found by the own-party session, which went from 112.83 money to 0 in one accepted command and was then told buying "costs 2 money". The resupply ceiling is now published as `party.resupplyTarget` |
| Only provisions can be bought, so the advertised trade ambition cannot be pursued, and one `work` tick out-earns the whole voyage | Design gap | Unassigned | Open; found by the own-party session. Arms, medicine and ship materials can never be purchased, so the wide spreads that do exist are unreachable, while every priced route sits at or below break-even once the sell haircut and travel consumption are counted |
