...
- **In review:** `feature/player-trade-verbs` (M16) at `4ff6512`, awaiting the voyage playtest record before a pull request is opened. 120 tests pass, typecheck is clean, the golden hashes are unchanged, and the prior merges onto `main` (`deeb0d1`, [PR 8](https://github.com/taia-0/Open-Era/pull/8)) are untouched by it.
...
- **Agent:** Cursor | **Branch:** `feature/player-trade-verbs` | **Playtest:** pending
- A player can name a good and a quantity and see the price before paying, on any of the four resources, with every limit quoted before it is hit. The voyage can now pay for itself; whether it out-earns working the same ticks is a separate, deferred milestone.
...
- **Left open** — the fresh-context voyage playtest was still running at this commit and is **not yet written to `docs/playtests/`**, so the branch must not be promoted until it is. To finish it: the world was served on port `4410` at seed 1847 over `--database .open-era/playtests/trade-voyage-001.sqlite`, which is outside Git and survives a disconnect; recover it with `WorldStore` and write the standard artifacts (`writeReports`) beside the record, as the other playtests do. The one-click `buy-provisions` top-up still has no cost preview. Whether a merchant should be able to learn a remote price at all, or must sail to acquire one, is undecided. Pacing is the deliberate deferral: one `work` tick still out-earns a voyage per tick elapsed, and rebalancing that would move the golden hashes, so it is scoped as the next economy milestone rather than folded in here.
- **Links** — [design record](docs/design/world-simulation.md#player-trade) | [open items](#open-items)
