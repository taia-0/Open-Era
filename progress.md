...
- **In review:** `feature/player-trade-verbs` (M16) at `c7045e3`, verified and with its playtest record written ([trade-voyage-001](docs/playtests/trade-voyage-001.md), `PROMOTE` for the solvent-voyage hypothesis), but **no pull request is open yet**. 120 tests pass, typecheck is clean, the golden hashes are unchanged, and the prior merges onto `main` (`deeb0d1`, [PR 8](https://github.com/taia-0/Open-Era/pull/8)) are untouched by it. The playtest's defect and design findings are recorded in [open items](#open-items); none of them is a simulation transition.
- **Last verified commit:** `main` at `deeb0d1`, merging [pull request 8](https://github.com/taia-0/Open-Era/pull/8) (the defect sweep) onto [pull request 7](https://github.com/taia-0/Open-Era/pull/7) (Milestone B, merged as `26b9b71`). Both gates passed on the runner as well as locally. 106 tests pass there; the branch above adds 14.
- **Gate status:** passing. Golden hashes lock pre-redaction behavior, still pass unchanged across four further milestones, and reproduce on the runner as well as locally, so the flagged ICU sensitivity has not materialized on the CI runtime. Both Milestone B and the sweep were deliberately behavior-neutral, so the three hashes are byte-identical to Milestone A's rather than regenerated. Bumping the actions to `v7` cleared the Node 20 deprecation notice; the only remaining runner notice is the pre-existing Ubuntu 26 migration on 2026-10-19, which is already tracked as the ICU risk below.
- **Headline risk:** no channel exists for a commander to learn a rival's strength other than a settlement-scoped forecast. The feed's missing history, the missing pre-commitment forecast, and a party's blindness to its own provisioning are all closed as of Milestones A and B; estimation through investigation is not. The maritime economy now has a player verb and a route that pays for itself as of M16, but its pacing is unreconciled: one `work` tick still out-earns a voyage per tick, so trade is solvent without being competitive.
...
| The `market` block carries the player's own `money`, `load`, `free` and `capacity`, so it reads as the market's cash and hold | Defect | Unassigned | Open; raised by the trade-loop session, which read `market.money: 108` as a counterparty credit limit that does not exist. Projection naming only |
| `capabilities.requests.commands.body` omits `resource` and `quantity`, which both trade verbs require | Defect | Unassigned | Open; raised by the trade-loop session, which had to guess both field names. The `actions[]` entry states preconditions without naming the fields |
| `characters[].knowledge[].observedTick` leaks negative ticks for reports seeded before the world began | Defect | Unassigned | Open; raised by the trade-loop session, which saw `-11` at tick 0. The sweep fixed this for briefing item ids, not for this raw field |
| `eventFeed` is a metadata envelope that contains no events; the events are under a sibling `events` key | Ergonomic | Unassigned | Open; raised by the trade-loop session, which paged `eventFeed` and got nothing. The name invites the wrong call |
