...
- **Last verified commit:** `7622bd3` on `main`, with Milestone B verified on `feature/own-party-observability`. 98 tests pass, typecheck is clean, and the gate passes locally for both. GitHub Actions is pending on the Milestone B pull request.
- **Gate status:** passing. Golden hashes lock pre-redaction behavior, still pass unchanged across three further milestones, and reproduce on the runner as well as locally, so the flagged ICU sensitivity has not materialized on the CI runtime. Milestone B was deliberately derivation-only, so its three hashes are byte-identical to Milestone A's rather than regenerated.
...
| `capabilities.requests` omits four endpoints: `/api/threads`, `/api/messages`, `/api/briefing/acknowledge` and `/api/briefing/officer` | Defect | Unassigned | Open; found by the own-party session, which concluded the conversation system was unreachable because nothing documented it. Both routes exist and work, so the contract block understates the API it claims to publish |
...
| `intel:*` briefing ids carry negative `observedTick` values, so stale-intelligence warnings fire on day one and report an age older than the world | Defect | Unassigned | Open; found while verifying this milestone. Seeded report knowledge is backdated `-rng.integer(3, 30)` at `scenario.ts:222`, which both trips the three-day staleness rule immediately and makes "the report is 28 ticks old" true of a world that is 2 days old |
